+++
title = "会社概要"
subtitle = "お問い合わせはこちらから。"
template = "single.html"
+++

## 会社名

デジノ株式会社

## 事業内容

スクラムに基づいたシステム開発の提供

1. Web アプリケーション開発
1. Web システム開発コンサルティング
1. Web 技術トレーニング
1. アジャイルコーチング

## 住所

〒150-0001
東京都渋谷区神宮前六丁目23番4号 桑野ビル２階

## Phone

050-3558‬‬‬-3502

## e-mail

<info@degino.com>

## 代表者名

徳冨 優一

## 資格

- Certified Scrum Product Owner® (2024/05/14)
- Certified Mobius Product Practitioner (2019/11)
- Certified ScrumMaster® (2011/06)
- 第一種情報処理
- 第二種情報処理

## 習得技術

| カテゴリ | 技術 |
|:--|:--|
| 主言語 | Python, Elixir |
| 経験言語 | TypeScript, React, Node.js, Perl, Ruby, Java, R, Solidity, C |
| | C++, ObjectiveC, PL/SQL, PL/1, COBOL, Pascal, UML, etc |
| フロントエンド | React, redux, NVD3, D3.js, Ext JS, HTML5 |
| クラウド | AWS, GAE (Python), Firestore |
| データベース | MySQL, PostgreSQL, Oracle, SQLite, Redis |
| データ分析 | Redshift, Spectrum, InfiniDB |
| その他 | Resium, Pandas, Sphinx, Tinkerer, Zola |

## 書籍寄稿

- Ultimate Agile Stories Iteration1
- UltimateAgileStories 10th Anniversary

## 登壇実績

### 2023 年

- 1 月 - Regional Scrum Gathering Tokyo 2023
  - [だったら WF でやればいいぢゃない! 〜 ところでホントに WF をご存知ですか? 〜](https://confengine.com/conferences/regional-scrum-gathering-tokyo-2023/proposal/17614)

### 2022 年

- 1月 - Regional Scrum Gathering Tokyo 2022
  - [その開発計画は最初から間違えている! - アジャイルにおける開発計画の考え方について](https://confengine.com/conferences/regional-scrum-gathering-tokyo-2022/proposal/15943)
- 5月 - スクラムフェス新潟 2022
  - [その品質は最初から間違っている! - アジャイルにおける品質の考え方について](https://confengine.com/conferences/scrum-fest-niigata-2022/proposal/16441)

### 2021 年

- 6月 - スクラムフェス大阪 2021
  - [ゲームのように学ぶアジャイル開発](https://confengine.com/conferences/scrum-fest-osaka-2021/proposal/15163)

## 開発実績

### 2022 年 xR 事業者様向け

- 3D 地図システム
- 役割: リードエンジニア、スクラムデベロッパー、スクラムアドバイザー
- 開発環境: React, TypeScript, PostgreSQL, Azure

### 2021年　某銀行様向け

- 温室効果ガス可視化システム
- 役割: リードエンジニア、スクラムデベロッパー、スクラムアドバイザー
- 開発環境: React, TypeScript, PostgreSQL, AWS

### 2020年 某人材サービス企業様向け

- 転職支援サービス
- 役割: プログラマー、スクラムアドバイザー
- 開発環境: React, kotlin, PostgreSQL

### 2019年 某アパレル製造小売業会社様向け

- デジタルサイネージシステム
- 役割: アーキテクト、リードエンジニア、スクラムデベロッパー、スクラムマスター
- 開発環境: React, nodejs, Firebase, AWS

### 2018年 某ソーシャルゲーム会社様向け

- 運営分析システム for Realtime
- 役割: アーキテクト、スクラムデベロッパー、システム管理者
- 開発環境: python, RedShift, DOMO, AWS

### 2017年 某ソーシャルゲーム会社様向け

- 運営分析システム for Native
- 役割: アーキテクト、スクラムデベロッパー、システム管理者
- 開発環境: python, React, Redshift, AWS

### 2014年 某ソーシャルゲーム会社様向け

- 運営分析システム
- 役割: アーキテクト、スクラムデベロッパー、システム管理者、スクラムマスター
- 開発環境: python, MySQL, ExtJS, AWS

### 2013年 某自動車メーカー会社様 R&D

- スマートグリッドシミュレーションシステム
- 役割: アーキテクト、スクラムデベロッパー
- 開発環境: Java, PostgreSQL, ExtJS, AWS

### 2012年 某臨床検査会社様向け

- 分注管理システム
- 役割: スクラムデベロッパー
- 開発環境: Java, MySQL, ExtJS

### 2010年 某コンサルティング会社様向け

- 給与分析システム
- 役割: アーキテクト、プログラマー
- 開発環境: python, ExtJS

### 2007年 モバイル大手企業様向け

- アフェリエイトシステム
- 役割: プログラマー
- 開発環境: perl, MySQL

### 2005年 未踏ソフトウェ ア創造事業

- Typical first 開発支援ツール "Ryoma"
- 役割: プログラマー
- 開発環境: ruby, MySQL, Flash

### 2004年 自動車会社様向け

- 輸送計画システム
- 役割: プログラマー
- 開発環境: Java

### 2000年 大手小売店様向け

- 営業分析システム
- 役割: プログラマー
- 開発環境: PL/SQL, Oracle
